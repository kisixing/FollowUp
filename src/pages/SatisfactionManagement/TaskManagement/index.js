...
export const category1 = ['全院', '科室', '医生', '其他'];

export const category2 = ['看诊', '住院', '检验检查', '咨询', '手术'];
...
export default () => {
  ...
  const { list, status, selectedScene, selectedType, loading, title } = state;
  ...
  const fetchData = () => {
    ...
    const scene = selectedScene.length === 1 ? selectedScene[0] : null;
    const type = selectedType.length === 1 ? selectedType[0] : null;
    const _status = status === 'all' ? null : parseInt(status, 10);
    request
      .post('/api/festivalConcern/list', { data: { scene, type, status: _status, title } })
      .then(data => {
  ...
  useEffect(() => {
    fetchData();
  }, [selectedScene, selectedType, status]);
  ...
  const tagsMap = tags => {
    const tagChild = tags.map(({ type, value }, index) => (
      ...
      <span key={index} style={{ display: 'inline-block' }}>
        ...
        >
          {type === 'selectedScene' ? category1[value] : category2[value]}
        </Tag>
      </span>
    ));
    return tagChild;
  ...
  const selectedItems = selectedScene
    .map(_ => ({ type: 'selectedScene', value: _ }))
